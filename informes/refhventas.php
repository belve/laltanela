...
$barrasIN .=$cd . ",";
$codigosREF[$cd]=$refprov;	

if(!array_key_exists($cd, $codigos)){
$codigos[$refprov]="$refprov";
...
$tiends[$idt]=1;
if(isset($enviados[$refprov][$idt])){
$enviados[$refprov][$idt]=$enviados[$refprov][$idt]+$cant;	
}else{
$enviados[$refprov][$idt]=$cant;
}
...
if(array_key_exists($refprov, $enviados)){
if(array_key_exists($idt, $enviados[$refprov])){	
$enviados[$refprov][$idt]=$enviados[$refprov][$idt]+$cant;	

}else{$enviados[$refprov][$idt]=$cant;	}
}else{$enviados[$refprov][$idt]=$cant;	}
}
########################################################
...
$codv=array();
$queryp= "select id_articulo, id_tienda, sum(cantidad) as cant, sum(importe * cantidad) as imp, 
(select refprov from articulos where codbarras=id_articulo) as refprov, 
importe from ticket_det where (fecha >= '$fini' AND fecha <= '$ffin')
...
$cods2 .=$cd . ","; 
			...
			if(isset($vendidos[$idt][$refprov]['c'])){
			$vendidos[$idt][$refprov]['c']=$vendidos[$idt][$refprov]['c']+$cant;	
			}else{
			$vendidos[$idt][$refprov]['c']=$cant;
			}
			
			if(isset($vendidos[$idt][$refprov]['i'])){
			$vendidos[$idt][$refprov]['i']=$vendidos[$idt][$refprov]['c']+$imp;	
			}else{
			$vendidos[$idt][$refprov]['i']=$imp;
			}
...
foreach ($tiendas as $idt => $value) {
	...
	$refprov=$codigosREF[$cd];
	
	if(isset($stocks[$idt][$refprov])){
	$stocks[$idt][$refprov]=$stocks[$idt][$refprov]+$cant;	
	}else{
	$stocks[$idt][$refprov]=$cant;
	}
